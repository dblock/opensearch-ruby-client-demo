...
begin
  ...
  client.http.post("/#{index}/_doc/1", body: document)

  # wait for the document to index
  sleep(3)

  # search for the document
  results = client.http.post("/#{index}/_search", body: { query: { match: { director: 'miller' } } })
  results['hits']['hits'].each do |hit|
    puts hit
  end

  # delete the document
  client.http.delete("/#{index}/_doc/1")
ensure
  # delete the index
  client.http.delete("/#{index}")
end
